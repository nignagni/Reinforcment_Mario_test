...
model = googlenet(WIDTH, HEIGHT, LR, output=4)
...
with tf.Session() as sess:
    ...
    for i in range(num_episodes):
        ...
        while j < 500:
            ...
            screen0 = np.array(screen0.reshape(-1,WIDTH,HEIGHT,1))
            allQ = np.asarray(model.predict(screen0))
            a = np.argmax(allQ)
            average = np.average(allQ)
            
            print('Just a test', a, 'With all of them ',allQ, "Whats the average? ", average)
            ...
            screen1 = np.array(compare.reshape(-1,WIDTH,HEIGHT,1))
            ...
            dist = np.linalg.norm(screen0-compare)/1000
            
            r = r + dist
            
            print ("reward" ,r)
            #Obtain the Q' values by feeding the new state through our network
            Q1 = np.asarray(model.predict(screen1))
            print ("New Q" ,Q1)
...
#            model.fit({'input': screen0}, {'targets': targetQ}, n_epoch=1, 
#                show_metric=True)
            model.fit({'input': X}, {'targets': targetQ}, n_epoch=1, validation_set=({'X_data':screen0 }, {'targets': targetQ}), 
